...
enum custom_keycodes {
    TEST = SAFE_RANGE,      //QK_KB_0 for custom keycodes
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
     if (record->event.pressed) {             //Code for macros
        switch(keycode) {
            ...
            case NUM:
            if (record->event.pressed) {
                SEND_STRING( SS_TAP(X_NUM) );                  //Toggles layer change and presses the num lock button, to allow for Num lock toggle to occure for both Mac OS and Windows
            return true;
            }break;
                //Discord Section
            case THIS:
